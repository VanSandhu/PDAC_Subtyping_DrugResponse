...
big_matrix= lapply(1:length(cellline_dataset), function(x) cluster_all(ccle))
...
###################### 22 cohorts run
cohorts$lunardi= impute.knn(data.matrix(cohorts$lunardi))[[1]]
cohorts$hamidi= impute.knn(data.matrix(cohorts$hamidi))[[1]]
big_matrix1 =list()
for( i in 1: length(cohorts)){
  print(paste("Clustering Dataset",i))
  
  big_matrix1[[i]]= cluster_all(cohorts[[i]])
